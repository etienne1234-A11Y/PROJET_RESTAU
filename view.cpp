...
bool View::connectToDatabase() {
    // Afficher les pilotes de base de données disponibles
    qDebug() << "Pilotes disponibles :" << QSqlDatabase::drivers();

    // Définir le chemin absolu vers la base de données
    QString dbPath = "C:/Users/hp/Documents/projet innovation/depot/cmake-build-debug/base_donnnees.db";

    // Débogage : Afficher le chemin de la base de données
    qDebug() << "Chemin de la BD : " << dbPath;

    // Vérification si le fichier existe
    if (!QFile::exists(dbPath)) {
        qDebug() << "Fichier de la base de données introuvable au chemin :" << dbPath;
        return false;
    }
